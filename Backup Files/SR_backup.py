...
def SR(Dh: np.ndarray, Dl: np.ndarray, Y: np.ndarray, blur_kernel: np.ndarray, upscale):
    ...
    for patch_num in tqdm(range(total_patches)):
        ...
        model, a = lasso_optimization(D_tilde, y_tilde, 0.1)
        print(f"y: {y}")
        print(f"D_tilde: {D_tilde}, y_tilde: {y_tilde}, a: {a}")
        print(f"R2 Score: {model.score(D_tilde, y_tilde)}")
        x = Dh @ a + m
        x = x.reshape(patch_shape, order = 'F')
        insert_patch(X0, patch_shape, stride, patch_num, x)
        
        print(f"Reconstructed Patch: {x}, Test1: {(Dh @ a).reshape(patch_shape, order = 'F')}, Mean: {m}")
        print("--------------------------------------------------------------------------------------------")
...
def lasso_optimization(D_tilde, y_tilde, lamb):
    ...
    return clf, clf.coef_.reshape((-1, 1))
